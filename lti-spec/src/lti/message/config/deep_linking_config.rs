...
pub struct DeepLinkingConfig {
    #[serde(skip_serializing_if = "Option::is_none")]
    #[serde(alias = "https://canvas.instructure.com/lti/course_navigation/default_enabled")]
    pub course_navigation: Option<bool>,

    #[serde(skip_serializing_if = "Option::is_none")]
    pub custom_parameters: Option<CustomParams>,

    #[serde(skip_serializing_if = "Option::is_none")]
    #[serde(alias = "https://canvas.instructure.com/lti/display_type")]
    ...
    #[serde(skip_serializing_if = "Option::is_none")]
    #[serde(alias = "https://canvas.instructure.com/lti/launch_height")]
    pub launch_height: Option<CanvasLaunchMeasurement>,

    #[serde(skip_serializing_if = "Option::is_none")]
    #[serde(alias = "https://canvas.instructure.com/lti/launch_width")]
    ...
    #[serde(skip_serializing_if = "Option::is_none")]
    pub supported_types: Option<Box<[Box<str>]>>,

    #[serde(skip_serializing_if = "Option::is_none")]
    pub supported_media_types: Option<Box<[Box<str>]>>,

    #[serde(skip_serializing_if = "Option::is_none")]
    pub target_link_uri: Option<Url>,

    #[serde(skip_serializing_if = "Option::is_none")]
    #[serde(alias = "https://canvas.instructure.com/lti/visibility")]
    pub visibility: Option<CanvasVisibility>,
}

impl DeepLinkingConfig {
    pub fn new() -> Self {
        Self::default()
    }
    ...
    pub fn add_supported_type(mut self, value: impl Into<Box<str>>) -> Self {
        let slice: Box<[Box<str>]> = self.supported_types.unwrap_or_default();

        let mut vec: Vec<Box<str>> = slice.to_vec();

        vec.push(value.into());

        self.supported_types = Some(vec.into_boxed_slice());

        self
    }

    pub fn add_supported_media_type(mut self, value: impl Into<Box<str>>) -> Self {
        let slice: Box<[Box<str>]> = self.supported_media_types.unwrap_or_default();

        let mut vec: Vec<Box<str>> = slice.to_vec();

        vec.push(value.into());

        self.supported_media_types = Some(vec.into_boxed_slice());

        self
    }

    pub fn set_course_navigation(mut self, value: bool) -> Self {
        self.course_navigation = Some(value);
